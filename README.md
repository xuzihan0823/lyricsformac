...
1. 网易云歌词 API（优先显示）
2. 本地 `.lrc` 文件
3. Apple Music 内嵌时间轴（若歌词本身带 `[mm:ss.xx]`）
...
- 右上角 `x` 按钮：关闭/隐藏悬浮窗，但不退出应用
- 顶部信息按钮：显示或隐藏当前歌曲名和歌手名
- 菜单栏图标：可重新显示悬浮窗、切换锁定/点击穿透/主题/透明度
- 菜单栏中的“退出 LyricsFloat”：完全退出应用
